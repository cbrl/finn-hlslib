		...
		switch (op) {
			...
			case ORAMOp::Write: {
				...
				if (it_bool.first != stash.end()) {
					auto& stash_block = it_bool.first.access(stash).second;
...
private:
	...
	void writePath(client_leaf_id leaf, uint8_t* server_data) {
		for (int16_t l = HeightL; l >= 0; --l) {
			...
			for (uint8_t z = 0; z < std::min(valid_cnt, BucketSizeZ); ++z) {
				...
				block.data = stash.at(block.id); //stash.emplace_empty(block.id).first.access(stash).second;
	...
	uint8_t getIntersectingBlocks(client_block_id (&valid_blocks)[(HeightL+1) * BucketSizeZ], client_leaf_id leaf, uint8_t height) {
		...
		for (auto it = stash.begin(); it != stash.end(); it.increment(stash)) {
			const client_block_id block_id = it.access(stash).first;
	...
	BinaryTree<client_block_id, Block, (util::ceil_int_log2(block_count_N) << 2)> stash; //size: HeightL * BucketSizeZ ?
